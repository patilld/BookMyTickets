            ...
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'nexus-cred',
                        ...
                        sh 'docker tag bookmytickets:latest http://65.0.168.23:8085/repository/bookmytickets/Test:1.1.${BUILD_NUMBER}'
                        sh 'docker login http://65.0.168.23:8085/ -u ${USERNAME} -p ${PASSWORD}'
                        sh 'docker push http://65.0.168.23:8085/repository/bookmytickets/Test:1.1.${BUILD_NUMBER}'
